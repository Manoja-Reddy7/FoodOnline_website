...
def place_order(request):
    ...
    for i in cart_items:
        if i.fooditem.vendor.id not in vendor_ids:
            vendor_ids.append(i.fooditem.vendor.id) # which resterant has linked with the respective food item.
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            order.vendors.add(*vendor_ids)  # *vendor_ids: The asterisk (*) is the unpacking operator in Python. It unpacks the list vendor_ids into individual arguments.
            order.save()
...
def payments(request):
        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
            ...
            # Clear the cart if payment is sucess.
